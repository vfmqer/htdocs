...
function Init(v1,v2,v3) {
    //打开xlmdocm文档
    xmlDoc = loadXmlFile("province.xml");
    ...
    if (TopnodeList.length > 0) {
        ...
        if (TopnodeList.length > 0) {
            ...
            if (province .length> 0) {
                //县列表
                //TopnodeList[0].childNodes.length 这个只有五个
                 city = province[childnodeindex].getElementsByTagName("county");//这个是全部地区
                 /* for (var i = 0; i < TopnodeList[0].childNodes.length; i++) {*/ // 病句，初始化时县区只有五个
                 for (var i = 0; i < city.length; i++) {
                    ...
                    this.document.getElementById("sel-area").add(option);
                }
